...
const DOCUMENTOS_GENERALES = [
  ...
  { key: 'facturaComercial', label: 'Factura Comercial de Importación' },
  { key: 'certificadoAnalisis', label: 'Certificados de Análisis Emitidos por la Empresa Fabricante' },
];

const DOCUMENTOS_POR_TIPO_PRODUCTO = [
  { key: 'certificadoBPM', label: 'Certificados de Buenas Prácticas de Manufactura del Laboratorio Fabricante' },
  { key: 'certificadoLibreVenta', label: 'Certificado de Libre Venta' },
];

export default function SolicitudImportacionMateriaPrimaFase02() {
  ...
  const todosDocumentosCargados = [...DOCUMENTOS_GENERALES, ...DOCUMENTOS_POR_TIPO_PRODUCTO].every(
    doc => documentos[doc.key]
  );
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      for (const doc of DOCUMENTOS_GENERALES) {
      ...
      // Subir documentos por tipo de producto
      for (const doc of DOCUMENTOS_POR_TIPO_PRODUCTO) {
        const file = documentos[doc.key];
        if (file) {
          await uploadDocument(requestId, file, { 
            tipo_documento: doc.label,
      ...
      console.error('Error durante el envío de fase 2:', error);
      alert(error?.message || 'Error al enviar la fase 2. Por favor, intenta de nuevo.');
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-[#85B6EC] font-semibold">FASE 02</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-white rounded-xl shadow-md border border-gray-200 p-8 mb-6">
            <h2 className="text-xl font-semibold text-gray-800 mb-2">Documentos según tipo de producto</h2>
            <p className="text-sm text-gray-600 mb-6">
              Para productos semielaborados, individualizados o granél destinados a la fabricación en laboratorios nacionales, debe anexar:
            </p>

            <div className="space-y-6">
              {DOCUMENTOS_POR_TIPO_PRODUCTO.map(doc => (
                <div key={doc.key}>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {doc.label}
                  </label>
                  <div className="flex gap-3">
                    <input
                      ...
                      value={documentos[doc.key]?.name || ''}
                      placeholder="Solo se permiten archivos PNG, JPG o PDF"
                      className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg bg-gray-50 text-gray-600 text-sm"
                    />
                    <input
                      ref={el => inputRefs.current[doc.key] = el}
                      type="file"
